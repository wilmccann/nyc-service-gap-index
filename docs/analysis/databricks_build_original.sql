-- =====================================================================
-- IS MY BLOCK CURSED?  --  Databricks build script
-- Paste each section into its own notebook cell, in order.
-- Raw tables assumed: workspace.default.rat_sightings
--                     workspace.default.restaurant_inspections
-- (rename below if yours differ)
-- Every CREATE cell also re-applies its COMMENTs, so a rebuild never
-- silently strips descriptions (Genie reads them).
-- =====================================================================


-- ---------------------------------------------------------------------
-- CELL 1: Clean 311 rodent complaints  (one row per complaint)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 2: Clean restaurant violations  (one row per violation, as delivered)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 3: Restaurants  (one row per restaurant)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 3b: Population by ZIP  (one row per ZIP; depends only on the raw upload)
-- Raw table: workspace.default.nyc_population_by_zip (zip_code INT, borough, population INT, margin_of_error INT)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 4: ZIP Service Gap Index  (one row per ZIP)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 5: Sanity checks (expected values from local profiling of the CSVs)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- CELL 6: Dashboard query (one ZIP parameter drives everything)
-- In the dashboard, create a text parameter named zip_param.
-- ---------------------------------------------------------------------
